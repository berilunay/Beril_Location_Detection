...
"""Extract image frame from the video"""
...
def extract_labels_and_views():
    video_name = "Video7"
    video_path = f"/home/beril/Thesis_Beril/Dataset_preprocess_new/Images/{video_name}"
    filenames = sorted(os.listdir(video_path))

    Parallel(n_jobs=10)(
        delayed(process_image_location)(video_name=video_name, current=current, filename=filename) for current, filename in
        enumerate(filenames))

"""Extracts all the labels and views for the location detection model. Saves the corresponding inputs into the copy_path"""
def process_image_location(video_name, current, filename):
...
def TI_model_labels():
    ...
    Parallel(n_jobs=10)(
        delayed(process_image_TImodel)(video_name=video_name, current=current, filename=filename) for current, filename
        in
        enumerate(filenames))

"""Prepares the input for the TI model"""
def process_image_TImodel(video_name, current, filename):
    im_path = f'/home/beril/Thesis_Beril/Dataset_preprocess_new/Images/{video_name}/' + str(filename)
...
def create_train_label_quality():
    ...
    Parallel(n_jobs=10)(
        delayed(process_image_quality)(video_name=video_name, current=current, filename=filename) for current, filename in
        enumerate(filenames))

"""prepares the quality detection pipeline input"""
def process_image_quality(video_name, current, filename):
    im_path = f'/home/beril/Thesis_Beril/Dataset_preprocess_new/Images/{video_name}/' + str(filename)
    ...
    """Removing the frames that do not have the proper labels"""
...
"""Prepration of the dataset for 3D CNN location detection model"""
...
def video_label_quality():
    ...
    Parallel(n_jobs=10)(
        delayed(create_video_folder_quality)(video_name=video_name, count=current, dir_name=filename) for
        current, filename in
        enumerate(filenames))

"""Prepration of the dataset for 3D CNN quality detection model"""
def create_video_folder_quality(video_name, count, dir_name):
    ...
    image_path = f'/home/beril/Thesis_Beril/Dataset_preprocess_new/Train_Quality_Labels/{video_name}/' + str(
        dir_name) + "/" + "colon.png"
    img = Image.open(image_path)
    label_path = f'/home/beril/Thesis_Beril/Dataset_preprocess_new/Train_Quality_Labels/{video_name}/' + str(
        dir_name) + "/" + "Quality.txt"
    loc = open(label_path, 'r')
    string_loc = loc.read()
...
"code for removing procedure label from the quality detection model"
def clean_P():
    video_name = "Video7"
...
if __name__ == '__main__':
    """any funtion implemented above, can be called separately for the corresponding task in here"""
   #conv_video2_image()
